...
void InputManager::updateGamepads()
{
    ...
    m_disconnectedPollCounter   = (m_disconnectedPollCounter + 1) % k_disconnectedPollInterval;
...
uint32_t InputManager::buildButtonMask(const XINPUT_GAMEPAD& gp)
{
    ...
    if (gp.bLeftTrigger  > k_triggerThreshold)
        mask |= static_cast<uint32_t>(GamepadButton::LeftTrigger);
    if (gp.bRightTrigger > k_triggerThreshold)
        mask |= static_cast<uint32_t>(GamepadButton::RightTrigger);

    return mask;
}

void InputManager::applyStickDeadzone(float rawX, float rawY, float deadzone,
...
{
    if (raw <= k_triggerThreshold)
        return 0.f;

    return static_cast<float>(raw - k_triggerThreshold)
         / static_cast<float>(255 - k_triggerThreshold);
}
...
//  バイブレーション
// =============================================================
void InputManager::setGamepadVibration(float leftMotor, float rightMotor,
                                       float duration, int index)
{
...
//  アクション バインディング
// =============================================================
void InputManager::bindAction(const std::string& actionName, uint8_t key,
                              float bufferTime)
{
...
void InputManager::bindAction(const std::string& actionName, GamepadButton button,
                              float bufferTime, int controllerIndex)
{
    auto& action = m_actionBindings[actionName];
...
void InputManager::bindAxis(const std::string& name, GamepadAxis axis,
                            int controllerIndex, bool invert)
{
    auto& a          = m_axes[name];
